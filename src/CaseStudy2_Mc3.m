...
% y_c[k]   = [1 0 0]
% x_c = [T_z1 T_w5  T_w2]^T
% u_c = T_sa
% d_c =[T_out T_hall CO2_1 T_rw,r1 ]^T
% -------------------------------------------------------
% -------------------------------------------------------
...
Ac      = zeros(3);
Ac(1,1) = -(2/(Zone1.Rn*Zone1.Cz))-Materials.air.Cpa*m1/Zone1.Cz - (Pout1*Radiator.alpha2)/Zone1.Cz;
...
Fc(1,5) = (Pout1*Radiator.alpha1 + Zone1.zeta)/Zone1.Cz;

Fc(2,1) = Zone1.alpha*Zone1.A_w*Zone1.iota/(Zone1.Cn) + 1/(Zone1.Rout*Zone1.Cn);
Fc(2,5) = Zone1.alpha*Zone1.A_w*Zone1.gamma/(Zone1.Cn) + (AHU.rw.alpha3*Trwass)/Zone1.Cn;

Fc(3,1) = (1/(Zone1.Rout*Zone1.Cn));
Fc(3,5) = (AHU.rw.alpha3*Trwass)/(Zone1.Cn);
...
D = [Tout Thall CO2_1 Trwr1 ones(T,1)];

% Simulate model over given time horizon T 
Tz1_y      = runModel(Z1m,[ 20 18 18 ]', Tsa,D,T);
